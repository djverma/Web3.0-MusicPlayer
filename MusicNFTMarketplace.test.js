...
describe("MusicNFTMarketplace", function () { 
  ...
  let URI = "https://bafybeialyfczpcph6j7s5djdoiurn6a2qpj2zyju36zchqwddhqdfxycpm.ipfs.nftstorage.link/"
  ...
  describe("Deployment", function () {
    ...
    it("Should mint then list all the music nfts", async function () {
      expect(await nftMarketplace.balanceOf(nftMarketplace.address)).to.equal(6);
    ...
    it("Ether balance should equal deployment fees", async function () {
     ...
     describe("Updating royalty fee",function (){
        it("Only owner should be able to update royalty fee",async function() {
            const fee=toWei(0.02)
            await nftMarketplace.updateRoyaltyFee(fee)
            await expect(
                nftMarketplace.connect(user1).updateRoyaltyFee(fee)
            ).to.be.revertedWith("Ownable: caller is not the owner");
            expect(await nftMarketplace.royaltyFee()).to.be.equal(fee)
        })
     })
     describe("Buying Tokens",function (){
      it("Should update seller to zero address, transfer NFT, pay seller, pay royalty fee to the artist and emit a MarketItemBought event",async function() {
          ...
          .withArgs(
            0,deployer.address,user1.address,prices[0]
          )
      ...
      it("Should fail when ether amount sent with transaction doesnt not equal asking price",async function (){
        await expect(
